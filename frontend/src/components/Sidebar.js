...
const Sidebar = ({
  ...
  const getConversationAvatar = (conversation) => {
    if (conversation.is_group) {
      return 'https://via.placeholder.com/40x40/3B82F6/FFFFFF?text=Г';
    } else {
      const otherParticipant = conversation.participants.find(p => p.id !== user.id);
      return otherParticipant?.avatar_url || `https://via.placeholder.com/40x40/6B7280/FFFFFF?text=${otherParticipant?.display_name?.[0] || '?'}`;
    }
  };
                ...
                filteredUsers.map((u) => (
                  ...
                  >
                    <div className="flex items-center">
                      <div className="relative">
                        <img
                          src={u.avatar_url || `https://via.placeholder.com/40x40/6B7280/FFFFFF?text=${u.display_name[0]}`}
                          alt={u.display_name}
                          className="w-10 h-10 rounded-full"
                        />
                      ...
                      <div className="ml-3">
                        <h4 className="text-sm font-medium text-gray-900">{u.display_name}</h4>
